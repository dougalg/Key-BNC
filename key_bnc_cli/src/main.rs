...
use counter::Counter;
use key_bnc_utils::stats::{dispersion_normalized, log_likelihood, odds_ratio};
use key_bnc_utils::utils::{collect, tokenize};
use std::env;
use std::fmt;
...
fn main() {
    ...
    let entries: Vec<CorpusPart> = entries
        .into_iter()
    ...
    let results: Vec<WordStats> = all_words_counts
        ...
        .map(|(word, count)| {
            ...
            let part_data: Vec<(f64, f64)> = entries
                .iter()
                .map(|entry| {
            ...
            WordStats {
                ...
                log_likelihood: log_likelihood(
                    count as f64,
                    total_num_tokens as f64,
                    count_in_bnc as f64,
                    total_num_tokens_in_bnc as f64,
                ),
...
impl fmt::Display for CorpusPart {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{1} <{0}>: {2} tokens, {3}% of total",
            self.id, self.name, self.percent_of_total, self.token_count
        )
    }
}
...
impl fmt::Display for WordStats {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{{ word: {0}, count: {1}, ll: {2}, or: {3}, dispersion: {4} }}",
            self.word, self.count, self.log_likelihood, self.odds_ratio, self.dispersion
        )
    }
}
...
fn process_file(entry: &DirEntry, id: i32) -> CorpusPart {
    ...
    let mut contents = read_to_string(entry.path()).expect("Could not load contents.");
