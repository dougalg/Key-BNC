pub fn log_likelihood(
    ...
    let expected_freq_1 = target_corpus_size * (target_freq + comparison_freq)
        / (target_corpus_size + comparison_corpus_size);
    let expected_freq_2 = comparison_corpus_size * (target_freq + comparison_freq)
        / (target_corpus_size + comparison_corpus_size);
...
pub fn dispersion_normalized(s_v: &[(f64, f64)], f: f64) -> f64 {
    let min_s = s_v.iter().map(|(s, _v)| s).cloned().fold(1.0, f64::min);
