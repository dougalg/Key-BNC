...
pub fn tokenize(normalized: &mut str) -> Vec<String> {
    let mut words = vec![];
    ...
    normalized.chars().for_each(|curr_char| {
        ...
        if is_new_word && curr_char.is_ascii_digit() {
        ...
        let should_end_word = match current_word_type {
            WordType::String => {
                curr_char.is_whitespace()
                    || STRING_WORD_BREAKS.contains(&curr_char)
                    || (curr_char != '\'' && curr_char.is_ascii_punctuation())
            }
            WordType::Number => {
                !curr_char.is_ascii_digit() && !ADDITIONAL_VALID_NUMERIC_CHARS.contains(&curr_char)
            }
        };

        let should_push_char = match current_word_type {
            WordType::String => {
                (curr_char == '\'' && !is_new_word && !should_end_word)
                    || !(curr_char.is_whitespace()
                        || STRING_WORD_BREAKS.contains(&curr_char)
...
fn clean_single_quotes(orig_word: &[char]) -> Vec<String> {
    ...
    orig_word.iter().rev().for_each(|curr_char| {
        if result.is_empty()
            && word.is_empty()
            && (STRING_WORD_BREAKS.contains(curr_char) || curr_char.is_ascii_punctuation())
        {
            return;
        }
        word.push(curr_char);
