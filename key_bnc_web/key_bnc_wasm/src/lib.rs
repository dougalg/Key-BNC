...
impl KeyBnc {
    ...
    pub fn remove_entry(&mut self, file_id: i32) {
        let entry = self.entries.get(&file_id).expect("File does not exist");
        self.total_num_tokens_in_user_corpus -= entry.token_count;
        self.user_corpus_words_counts -= entry.word_counts.clone();
    ...
    pub fn get_stats(&mut self) -> JsValue {
        ...
        let entry_percentages: HashMap<&i32, f64> = self
            .entries
            .iter()
            .map(|(id, entry)| {
                (
                    id,
                    entry.token_count as f64 / self.total_num_tokens_in_user_corpus as f64,
                )
            })
            .collect();
        let res: Vec<WordStats> = self
            ...
            .map(|(word, count)| {
                ...
                let part_data: Vec<(f64, f64)> = self
                    .entries
                    .iter()
                    .map(|(id, entry)| {
...
pub fn get_bnc_count(bnc_raw_csv: String) -> HashMap<UniCase<String>, usize> {
    ...
    rdr.records()
        ...
        .map(|rec| {
            (
                UniCase::new(rec[2].parse::<String>().unwrap()),
                rec[1].parse::<usize>().unwrap(),
            )
        })
        .collect()
}
