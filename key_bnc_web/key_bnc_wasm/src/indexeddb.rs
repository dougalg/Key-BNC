...
pub async fn get_file_text(key: JsValue) -> Result<JsValue, JsValue> {
	// Open my_db v1
	let mut db_req: OpenDbRequest = IdbDatabase::open_u32(DB_NAME, 1)?;
	...
	let saved_file_text: Option<JsValue> = store.get_owned(key)?.await?;

	// IDBTransactions can have an Error or an Abort event; into_result() turns both into a
	// DOMException
	let _ = tx;

	saved_file_text.ok_or(JsValue::from(""))
}
